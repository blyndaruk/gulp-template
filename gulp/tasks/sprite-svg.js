...
gulp.task('sprite:svg', () => gulp
  ...
  .pipe(
    gulpcheerio({
      run: function($, file) {
        ...
        $('[stroke]:not([stroke="currentColor"])').removeAttr('stroke');
        // $('[stroke="currentColor"]').attr('fill', 'none');
        ...
        if ($svg.attr('height')){
          ...
          [w, h] = $svg.attr('viewbox').split(' ').splice(2);
  ...
  .pipe(through2.obj(function (file, encoding, cb) {
    ...
    let data = $('svg > symbol').map(function() {
      ...
      let stroke = $this.find('[stroke]:not([stroke="currentColor"])').attr('stroke');
